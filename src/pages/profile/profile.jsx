...
function Profile() {
  ...
  useEffect(() => {
    if (token) {
      ...
      fetch(`${process.env.REACT_APP_API_URL}/backend/api/profile.php`, {
        method: "GET",
        headers: {
  ...
  const handleConfirm = (fieldName, updatedSkills = null) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/backend/api/updateProfile.php`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    if (token) {
      ...
      try {
        const response = await fetch(`
          ${process.env.REACT_APP_API_URL}/backend/api/uploadImage.php`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          },
